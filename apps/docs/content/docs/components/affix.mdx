...
```tsx
<Affix offset={64} position="top" role="navigation" aria-label="Sticky navigation">
...
Accepts all standard HTML div attributes (e.g., `role`, `aria-label`, `id`).

## Accessibility

- **ARIA support** — Pass `role` and `aria-label` props directly for screen reader context
- **Data attribute** — `data-affixed` attribute is set when content is affixed (useful for CSS hooks)
- **Semantic HTML** — Uses `<div>` as a neutral container; wrap children semantically
- **Placeholder** — Uses `aria-hidden="true"` on the placeholder element to hide it from screen readers
