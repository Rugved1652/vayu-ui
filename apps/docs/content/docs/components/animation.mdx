...
```bash
npx vayu-ui-cli@latest add animation
```
...
// InView — viewport-triggered animation (client component)
<Animation.InView variant="fade" duration="slow">
  {children}
</Animation.InView>

<Animation.InView variant="slide" direction="up" triggerOnce={false}>
  {children}
</Animation.InView>
```
...
- **Animation.InView** — Viewport-triggered animation that plays when the element scrolls into view. A client component that wraps children with an IntersectionObserver. Supports all animation variants via the `variant` prop.

## Accessibility

- **Reduced Motion Support**:
  - Respects the OS-level `prefers-reduced-motion: reduce` setting via Tailwind's `motion-reduce:` CSS variant.
  - When enabled, animations are suppressed and content is rendered with `opacity: 1` via `motion-reduce:opacity-100` to ensure visibility.
  - A global CSS rule (`@media (prefers-reduced-motion: reduce) { * { animation: none !important } }`) provides an additional safety net.
...
├── AnimateInView.tsx          # InView variant (client component, viewport-triggered)
...
| Prop          | Type                                                                                                   | Default    | Description                                                     |
| :------------ | :----------------------------------------------------------------------------------------------------- | :--------- | :-------------------------------------------------------------- |
| `variant`     | `'fade' \| 'slide' \| 'bounce' \| 'flip' \| 'rotate' \| 'zoom' \| 'roll' \| 'jackInTheBox' \| 'hinge'` | —          | Animation type to play when the element enters the viewport.    |
| `direction`   | `'up' \| 'down' \| 'left' \| 'right'`                                                                  | `'left'`   | Direction for `slide`, `flip`, and `roll` variants.             |
| `scale`       | `'small' \| 'medium' \| 'large'`                                                                       | `'medium'` | Scale intensity for `bounce` and `zoom` variants.               |
| `degrees`     | `number`                                                                                               | `-200`     | Starting rotation angle for `rotate` variant.                   |
| `triggerOnce` | `boolean`                                                                                              | `true`     | Whether to animate only the first time the element enters view. |
| `threshold`   | `number`                                                                                               | `0.1`      | Intersection ratio (0–1) needed to trigger the animation.       |
| `rootMargin`  | `string`                                                                                               | `'0px'`    | Margin around the root element for IntersectionObserver.        |

Plus all [Common Props](#common-props-all-variants).

**Note:** `Animation.InView` is a client component (uses `IntersectionObserver`). Children passed to it can still be server-rendered.
