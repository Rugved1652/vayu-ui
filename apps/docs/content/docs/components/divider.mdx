...
- **Semantic vs Decorative**: By default, dividers render with `role="separator"` and `aria-orientation` for screen readers. Set `decorative={true}` to mark the divider as purely decorative. When the divider has children (labels), no role is applied as the content is readable.
- **Color Contrast**: All colors use WCAG-compliant tokens ensuring at least 3:1 contrast ratio for borders (using `ground-400` for better visibility) and 4.5:1 for text labels.
