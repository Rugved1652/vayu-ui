...
```tsx
<Modal>
  <Modal.Trigger>{/* Trigger button */}</Modal.Trigger>

  <Modal.Content>
    <Modal.Header>
      <Modal.Title>{/* Heading */}</Modal.Title>
      <Modal.Description>{/* Subtext */}</Modal.Description>
      <Modal.Close />
    </Modal.Header>
...
- `role="dialog"` with `aria-modal="true"` — screen readers announce modal context
- `aria-labelledby` linked to `Modal.Title` via auto-generated `useId()`
- `aria-describedby` linked to `Modal.Description` via auto-generated `useId()`
- Focus trap — Tab / Shift+Tab cycles within the modal
- Focus return — on close, focus is restored to the trigger element
- Escape key dismisses the modal (configurable via `closeOnEscape`)
- Body scroll lock when modal is open (compensates for scrollbar width shift)
- Overlay click dismisses the modal (configurable via `closeOnOverlayClick`)
- Close button includes `aria-label="Close"` for icon-only usage
- `sr-only` text provided inside the default close button for screen readers
- Content rendered via `createPortal` to `document.body` to avoid z-index clipping
- Visible focus indicators using `focus-visible:ring-2` across all interactive elements
...
| :--- | :--- | :--- | :--- |
| `open` | `boolean` | `undefined` | Controlled open state. When provided, the component becomes controlled. |
...
| Prop | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| `asChild` | `boolean` | `false` | Render as the child element instead of a default button. |
...
Renders as `<h2>`. Automatically linked to `aria-labelledby` on the dialog.
...
Renders as `<p>`. Automatically linked to `aria-describedby` on the dialog.
...
When no children are provided, renders a default X icon button with `aria-label="Close"`.
