...
- **Keyboard navigation**:
  ...
  - Focus trap inside mobile menu with visible element filtering
- **Focus management**:
  - Focus returns to toggle button when menu closes (WCAG 2.4.3)
  - Initial focus moves to close button when menu opens
...
- **Body scroll lock**:
  ...
  - Applies `inert` attribute to main content for complete focus containment
...
| `mainContentSelector` | `string` | `"main"` | CSS selector for main content element (gets inert attribute when mobile menu is open) |
