...
const PopoverContent = forwardRef<HTMLDivElement, PopoverContentProps>(
  (
    ...
    const variantClasses = {
      ...
      bordered: 'bg-ground-50 dark:bg-ground-900 border-2 border-primary-500 shadow-outer',
      elevated:
    ...
    useLayoutEffect(() => {
      if (open && triggerRef.current && contentRef.current) {
        ...
        if (showArrow) {
          ...
          if (finalSide === 'top' || finalSide === 'bottom') {
            arrowLeft = triggerRect.left + triggerRect.width / 2 - left - arrowSize;
    ...
    return (
      <>
        ...
        >
          ...
          <div className="font-secondary text-ground-800 dark:text-ground-200">{children}</div>
        </div>
      </>
    );
  },
