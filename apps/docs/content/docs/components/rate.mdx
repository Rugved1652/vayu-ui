      ...
      return (
        <div className="w-full max-w-md space-y-8">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-ground-500">Current value: {value}</p>
          </div>
...
| `color` | `'primary' \| 'secondary' \| 'warning' \| 'error' \| 'info' \| 'success'` | `'warning'` | Color theme of the stars. |
...
The Rate component is built with WCAG 2.2 AA compliance in mind:

- **Keyboard Navigation**: Full support for Arrow keys (Left/Right/Up/Down), Home, and End keys
- **Focus Visible**: Clear focus indicator with ring styling when the component receives keyboard focus
- **ARIA Roles**: Uses `role="slider"` with proper `aria-valuemin`, `aria-valuemax`, and `aria-valuenow` attributes
- **Screen Reader Support**: Proper `aria-label` and `aria-labelledby` support, with `aria-live` regions for dynamic text labels
- **Read-only and Disabled States**: Proper `aria-readonly` and `aria-disabled` attributes
