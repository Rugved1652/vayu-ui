    ...
    export default function TabsExample() {
      return (
        <div className="w-full max-w-md">
          {/* Horizontal tabs with aria-labelledby */}
...
- **ARIA Attributes**: Proper roles (`tablist`, `tab`, `tabpanel`), states (`aria-selected`, `aria-disabled`), and properties (`aria-controls`, `aria-labelledby`)
- **Roving Tabindex**: Only the active tab is in the tab order, arrow keys move focus between tabs
- **Screen Reader Support**: Use `aria-label` or `aria-labelledby` on `Tabs.List` to provide an accessible name
- **Disabled States**: Disabled tabs include both `disabled` attribute and `aria-disabled` for full screen reader support
...
| `autoFocus` | `boolean` | `false` | Automatically move focus to the tab panel when activated. |
...
| Prop | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| `aria-label` | `string` | - | Accessible label for the tab list. Required for WCAG 2.2 AA. |
| `aria-labelledby` | `string` | - | Alternative to aria-label, references an element that labels the tab list. |
| `className` | `string` | - | Additional CSS classes. |
