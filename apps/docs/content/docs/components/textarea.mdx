...
<TextArea.Root>
  ...
  <TextArea.Input placeholder="Enter text..." />
  <TextArea.SupportText>Helper text goes here</TextArea.SupportText>
  <TextArea.ErrorText>Error message</TextArea.ErrorText>
  <TextArea.CharCount />
</TextArea.Root>
```
...
| :--- | :--- | :--- | :--- |
| `variant` | `"default" \| "outline"` | `"default"` | Visual variant of the text area. |
...
| Prop | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| `showCharCount` | `boolean` | `false` | Whether to show character count next to the label. |
...
Displays character count based on context from `TextArea.Root`.
...
- Full keyboard navigation support with Tab to focus
- Visible focus indicators with `focus-visible:ring-2` styling
- Proper `aria-invalid` attribute when in error state
- `aria-required` attribute when `required` prop is set
- `aria-describedby` links input to support and error text
- `aria-errormessage` specifically points to error text when invalid
- `role="alert"` on error messages for screen reader announcements
- `aria-live="polite"` regions for character count updates
- `aria-disabled` attribute when disabled
- Semantic HTML with proper `label`/`input` associations via `htmlFor`/`id`
- `role="group"` with `aria-labelledby` on root for proper grouping
- Support for `prefers-reduced-motion` for transitions
