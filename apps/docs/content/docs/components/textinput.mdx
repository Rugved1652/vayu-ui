---
title: Text Input
description: A comfortable and robust text input component.
---
...
| `inputType` | `InputType` | `"text"` | Type of input (text, password, email, etc.) |
...
| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `leftIcon` | `ReactNode` | - | Icon to show on the left (deprecated, use TextInput.Icon) |
| `rightIcon` | `ReactNode` | - | Icon to show on the right (deprecated, use TextInput.Icon) |
...
```tsx title="src/components/ui/textinput.tsx"
"use client";
...
const Input = forwardRef<HTMLInputElement, InputProps>(
    ({ className = "", leftIcon, rightIcon, ...props }, ref) => {
        ...
        return (
            <>
                {leftIcon && (
                    <span className="text-ground-400 flex-shrink-0">{leftIcon}</span>
                )}
                <input
                    ref={mergedRef}
                    id={inputId}
          ...
          `}
                ...
                />
                {rightIcon && (
                    <span className="text-ground-400 flex-shrink-0">{rightIcon}</span>
                )}
            </>
        );
    }
