---
title: Typography
description: Text styles for headings, paragraphs, links, and inline code with Accessible contrast compliance.
---
...
- **Contrast compliance** — All color variants use design tokens that meet Accessible contrast ratios (4.5:1 for normal text, 3:1 for large text)
...
| `unsized`         | `boolean`                                                                                             | `false`     | Skip baked-in responsive size classes so className fully controls sizing. |
...
### Overriding Heading Sizes

Headings bake in responsive size classes (e.g. `text-2xl sm:text-3xl lg:text-4xl` for H3). `twMerge` resolves conflicts **within** the same breakpoint, but **not across breakpoints**. That means a consumer `className="text-sm"` only overrides the base size — the `sm:` and `lg:` classes survive and kick in at their breakpoints.

Use the `unsized` prop to opt out of the default sizing block entirely:

```tsx
{/* Stays text-sm on all breakpoints */}
<Typography.H3 unsized className="text-sm font-semibold">
  Small heading everywhere
</Typography.H3>

{/* Without unsized: balloons back to 2xl/3xl/4xl — don't do this */}
