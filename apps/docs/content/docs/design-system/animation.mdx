...
<Tabs items={['Preview', 'Code']}>
  <Tab value="Preview">
    <div className="p-6 bg-surface rounded-surface h-24 relative overflow-hidden">
      ...
      <div className="animate-float-particle absolute bottom-0 left-1/2 w-2 h-2 bg-brand rounded-full" style={{ animationDelay: '1s' }}></div>
      <div className="animate-float-particle absolute bottom-0 left-3/4 w-2 h-2 bg-brand rounded-full" style={{ animationDelay: '2s' }}></div>
    </div>
  </Tab>
  <Tab value="Code">
    ```tsx
