---
title: Vayu UI MCP Usage
description: Use the Vayu UI MCP server as the source of truth for choosing, installing, composing, and coding Vayu UI components and hooks.
---

# Vayu UI MCP Usage

This skill teaches AI assistants to use the Vayu UI MCP server as the source of truth. It ensures components are chosen, composed, and styled correctly by querying MCP tools first instead of guessing names, props, or variants from memory.

```bash
npx skills add Rugved1652/vayu-ui/mcp-usage
```
