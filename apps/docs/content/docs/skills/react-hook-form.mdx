---
title: RHF + Zod Validation Patterns
description: Enforces strict implementation patterns for forms built with React Hook Form, Zod schemas, and TanStack Query mutations.
---
...
}).refine((data) => data.type !== 'sms' || data.phoneNumber?.length > 0, {
...
<Controller
  ...
  control={control}
  render={({ field }) => (
    <SelectComponent value={field.value} onValueChange={field.onChange} onBlur={field.onBlur} />
  )}
/>
```
