...
export function UseHoverDemo() {
  ...
  return (
    <div className="p-6 border rounded-lg bg-card text-card-foreground shadow-sm w-full max-w-md mx-auto flex flex-col items-center gap-6">
      ...
      >
        <MousePointer2
          className={`w-10 h-10 transition-transform duration-300 ${isHovered ? 'scale-110' : 'scale-100'}`}
        />
        <span className="font-bold text-lg">{isHovered ? 'I am Hovered!' : 'Hover Me!'}</span>
      </div>
