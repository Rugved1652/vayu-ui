            ...
            <div className="space-y-2">
                ...
                <div className="min-h-[100px] bg-muted/20 rounded-md border p-2 flex flex-wrap gap-2 content-start">
                    {Array.from(set).map((item) => (
                        <div key={item} className="px-2 py-1 bg-background border rounded text-sm shadow-sm animate-in fade-in zoom-in-95 duration-200 flex items-center gap-2 group">
                            <span>{item}</span>
                            <button
                                onClick={() => remove(item)}
