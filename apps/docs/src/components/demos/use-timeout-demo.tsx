...
export function UseTimeoutDemo() {
    ...
    return (
        <div className="p-6 border rounded-lg bg-card text-card-foreground shadow-sm w-full max-w-md mx-auto flex flex-col items-center gap-6">
            <TimeoutInner key={trigger} onRestart={handleRestart} statusParent={status} setStatusParent={setStatus} />
        </div>
    );
}

function TimeoutInner({ onRestart, statusParent, setStatusParent }: { onRestart: () => void, statusParent: string, setStatusParent: (s: string) => void }) {
    ...
    return (
        <>
            <div className="flex flex-col items-center gap-2">
                <span className="text-sm text-muted-foreground uppercase tracking-wider font-semibold">Timeout Status</span>
                <span className={`text-4xl font-black tabular-nums ${statusParent === "Executed!" ? "text-green-500" : "text-yellow-500"}`}>
                    {statusParent}
                </span>
