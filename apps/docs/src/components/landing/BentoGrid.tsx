import { ArrowRight, Box, CreditCard, LayoutTemplate, MousePointerClick, Table, Terminal } from 'lucide-react';
...
function BentoCard({
    ...
    return (
        <div
            className={`relative overflow-hidden rounded-xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-950/50 p-6 shadow-sm transition-all hover:bg-neutral-50 dark:hover:bg-neutral-900/50 hover:shadow-md ${className}`}
        >
            <div className="absolute -right-4 -top-4 h-32 w-32 rounded-full bg-lime-500/10 blur-3xl" />
            <div className="relative z-10 flex flex-col h-full justify-between">
                <div>
                    <div className="mb-4 inline-flex h-10 w-10 items-center justify-center rounded-lg bg-lime-100 dark:bg-lime-500/10 text-lime-600 dark:text-lime-400">
                        <Icon className="h-5 w-5" />
                    </div>
                    <h3 className="mb-2 text-lg font-semibold text-neutral-900 dark:text-white">{name}</h3>
...
export function BentoGrid() {
    return (
        <div className="py-24 sm:py-32 bg-white dark:bg-neutral-950 transition-colors duration-300">
