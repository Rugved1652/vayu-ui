...
const steps = [
  ...
  },
  {
    ...
    description: 'Predictable patterns that grow with your product — and your AI tools.',
    icon: Layers,
    code: null,
  ...
  return (
    <section className="relative isolate overflow-hidden bg-linear-to-b from-surface via-canvas to-surface dark:from-canvas dark:via-surface dark:to-canvas px-6 py-24 sm:py-32 lg:px-8">
      ...
      <div className="absolute inset-0 -z-10 pointer-events-none overflow-hidden">
        ...
        <div
          className="absolute bottom-1/3 left-[8%] animate-pulse"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 lg:gap-8">
            {steps.map((step, index) => (
              <div key={step.number} className="relative group">
                {/* Card glow */}
                <div className="absolute inset-0 rounded-3xl bg-linear-to-br from-lime-500/10 to-cyan-500/5 blur-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-slow" />

                <div className="relative flex flex-col items-center text-center p-8 rounded-3xl border border-border/50 bg-elevated/80 dark:bg-surface/50 backdrop-blur-surface transition-all duration-medium hover:border-border hover:shadow-elevated hover:-translate-y-2">
                  {/* Step number badge */}
                  <div className="absolute -top-4 left-1/2 -translate-x-1/2">
                    <div
                      className={`flex h-8 w-8 items-center justify-center rounded-full bg-linear-to-r ${step.gradient} text-white text-xs font-bold shadow-lg`}
                    >
                      {step.number}
                    </div>
                  </div>
                  ...
                  <div className={`mt-4 mb-6 relative`}>
                    <div
                      className={`absolute inset-0 blur-2xl bg-linear-to-r ${step.gradient} opacity-30 group-hover:opacity-50 transition-opacity duration-300`}
                    />
                    <div
                      className={`relative flex h-16 w-16 items-center justify-center rounded-overlay bg-linear-to-br ${step.gradient} p-0.5`}
                    >
                      <div className="flex h-full w-full items-center justify-center rounded-[14px] bg-white dark:bg-neutral-900">
                        <step.icon
                          className={`h-7 w-7`}
                          style={{
                            color: step.gradient.includes('lime')
                              ? '#84cc16'
                              : step.gradient.includes('cyan')
                                ? '#06b6d4'
                                : '#a855f7',
                          }}
                        />
                  ...
                  {/* Content */}
                  <h3 className="font-primary text-xl font-semibold text-canvas-content mb-3">
                    {step.title}
                  </h3>

                  <p className="font-secondary text-sm text-surface-content leading-relaxed max-w-xs">
                    {step.description}
                  </p>
                  ...
                  {step.code && (
                    <div className="mt-6 w-full">
                      <div className="relative group/code">
                        ...
                        <code className="relative flex items-center justify-between gap-2 px-4 py-3 rounded-xl bg-neutral-900 dark:bg-neutral-800 text-lime-400 dark:text-lime-300 text-sm font-tertiary border border-border">
                          <span>{step.code}</span>
                          <button className="opacity-0 group-hover/code:opacity-100 transition-opacity duration-fast p-1 hover:bg-neutral-700 rounded">
                ...
                {index < steps.length - 1 && (
                  <div className="hidden lg:flex absolute top-1/2 -right-4 z-10 h-8 w-8 items-center justify-center rounded-full bg-elevated border border-border shadow-surface">
        ...
        <div className="mt-20 text-center">
          <div className="inline-flex flex-col sm:flex-row items-center gap-4">
            ...
            <Link
              ...
              className="group inline-flex items-center gap-2 rounded-control border-2 border-border px-6 py-3 text-sm font-semibold text-surface-content transition-all duration-fast hover:border-border hover:text-canvas-content"
            >
