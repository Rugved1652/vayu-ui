...
`;
    ...
    async run(): Promise<void> {
        ...
        for (const current of itemsToInstall) {
            ...
            const itemToInstall =
                flags.path && current.slug === slug
                    ? { ...current, targetPath: flags.path }
                    : current;
            ...
            if (result.alreadyExists && !flags.overwrite) {
                spinner.warn(
                    chalk.yellow(`${current.fileName} already exists — skipped`) +
                    chalk.dim(` (use --overwrite to replace)`)
                );
            } else {
                const relativePath = path.relative(process.cwd(), result.filePath);
                spinner.succeed(
                    chalk.green(`${current.fileName}`) +
        ...
        this.log(
            chalk.white(`  import { ${item.name} } from "@/hooks/${item.fileName.replace(".ts", "")}";`)
        );
        this.log("");
    }
}