...
`;
    ...
    async run(): Promise<void> {
        ...
        scanSpinner.succeed(
            chalk.green(`Found ${installed.length} installed item${installed.length === 1 ? "" : "s"}`)
        );
        ...
        for (const result of fetchResults) {
            if (result.status === "rejected") {
                ...
                this.log(
                    chalk.red("  ✖ ") + chalk.dim(`Failed to fetch: ${err.message}`)
                );
                continue;
            }
            ...
            if (isDryRun) {
                this.log(
                    ...
                    chalk.dim(` (${item.type}) — would update`)
                );
                updatedCount++;
            } else {
                try {
        ...
        if (upToDate > 0) {
            this.log(
                chalk.dim(`  ─ ${upToDate} item${upToDate === 1 ? "" : "s"} already up to date`)
            );
        }
    ...
    private async updateCss(cwd: string, isDryRun: boolean): Promise<void> {
        ...
        if (isDryRun) {
            this.log(
                chalk.yellow("  ↻ ") +
                chalk.white(cssPath) +
                chalk.dim(" — would update CSS tokens")
            );
        } else {
