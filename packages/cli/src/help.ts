...
export default class VayuHelp extends Help {
  protected async showRootHelp(): Promise<void> {
    ...
    this.log('')
    this.log(
      ux.colorize('cyan', '  ◆ ') + ux.colorize('bold', 'Vayu UI CLI') + ux.colorize('dim', ` v${this.config.version}`),
    )
    ...
    this.log(
      `    ${ux.colorize('dim', '$')} ${ux.colorize('bold', `${bin} <command>`)} ${ux.colorize('dim', '[options]')}`,
    )
    this.log('')
  ...
  public async showCommandHelp(command: any): Promise<void> {
    ...
    if (command.flags && Object.keys(command.flags).length > 0) {
      ...
      for (const [, flag] of Object.entries(command.flags as Record<string, any>)) {
        ...
        const defaultLabel =
          flag.default !== undefined && flag.default !== false ? ux.colorize('dim', ` (default: ${flag.default})`) : ''
