...
export async function installItem(item: RegistryItem, options: {overwrite?: boolean} = {}): Promise<InstallResult> {
  const cwd = process.cwd()
 ...
 */
export async function downloadAndExtractRepo(repo: string, targetDir: string, branch = 'main'): Promise<void> {
