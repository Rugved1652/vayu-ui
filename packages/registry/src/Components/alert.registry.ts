...
export const alertRegistry: VayuComponentDoc = {
  ...
  description:
    'Displays a callout for user attention with semantic variants. A compound component that provides contextual feedback messages for user actions or system status.',
  ai_summary:
    'A semantic feedback component with four variants (info, success, warning, error). Compound structure with Icon, Title, Description, Content, and Dismiss subcomponents. Supports WCAG 2.2 AA accessibility with appropriate ARIA roles and live regions.',
  ...
  props: {
    ...
    'Alert.Icon': [
      ...
      },
      {
        ...
        required: true,
        description:
          'The icon component to display (e.g., Info, CheckCircle, TriangleAlert, XCircle)',
      },
      {
        name: 'className',
        type: 'string',
  ...
  variants: [
    {
      ...
      default: 'info',
      description:
        'Semantic color variants: info (blue) for informational messages, success (green) for positive feedback, warning (yellow) for cautionary messages, error (red) for critical alerts',
    },
  ],
...
<Alert variant="info">
  ...
  installation: ['npx vayu-ui init', 'npx vayu-ui add alert'],
