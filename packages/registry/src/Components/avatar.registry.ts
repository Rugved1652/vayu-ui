export const avatarRegistry = {
  ...
  ai_summary:
    'A compound component for displaying user profile images with automatic fallback support. Renders initials with deterministic colors when images fail, supports status indicators, and provides full WCAG 2.2 AA accessibility compliance.',
    ...
    Avatar: [
      {
        ...
        default: "'medium'",
        description:
          'The size of the avatar container. Accepts predefined sizes or custom pixel value.',
      },
      {
        name: 'username',
        type: 'string',
      ...
      },
      {
        ...
        required: false,
        description:
          'Handler for interactive avatars. Makes avatar focusable and keyboard accessible.',
      },
      {
        name: 'tabIndex',
        type: 'number',
        required: false,
        description:
          'Custom tab index for keyboard navigation. Only applies when onClick is provided.',
      },
      {
        name: 'className',
        type: 'string',
  ...
  design_tokens: {
    ...
    recommended: {
      colors: ['ground-100', 'ground-800', 'primary-600'],
      radius: ['rounded-full'],
    },
    allowed: {
      colors: ['ground-*', 'primary-*', 'success-*', 'warning-*', 'error-*'],
      radius: ['rounded-full'],
      border: ['border-2'],
    },
  },
...
>
  ...
  installation: ['npx vayu-ui init    # Add Theme CSS if not added', 'npx vayu-ui add avatar'],
