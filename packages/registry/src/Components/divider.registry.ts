...
export const dividerRegistry: VayuComponentDoc = {
  ...
  description:
    'Visually or semantically separates content sections with horizontal or vertical lines, supporting labels and multiple visual variants.',
  ai_summary:
    'A compound component for creating visual or semantic separators. Supports horizontal and vertical orientations, labeled dividers, multiple line styles (solid, dashed, dotted), and WCAG-compliant color tokens. True server component with no client-side JavaScript required.',
  ...
  props: {
    Divider: [
      ...
      },
      {
        ...
        default: false,
        description:
          'If true, the divider is purely decorative (aria-hidden). If false, renders as semantic separator with ARIA role',
      },
      {
        name: 'children',
        type: 'ReactNode',
        required: false,
        description:
          'If provided, used as content (flex container). If not, renders a default line',
      },
      {
        name: 'className',
        type: 'string',
  ...
  installation: ['npx vayu-ui init    # Add Theme CSS if not added', 'npx vayu-ui add divider'],
