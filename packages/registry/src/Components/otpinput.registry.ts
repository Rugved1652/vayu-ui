...
export const otpinputRegistry: VayuComponentDoc = {
  ...
  description:
    'An accessible one-time password input component with compound component pattern for entering verification codes, PINs, and authentication tokens.',
  ai_summary:
    'A compound OTP input component that provides an accessible, customizable interface for entering one-time passwords. Uses a hidden single input with visual slot representation for better UX while maintaining full accessibility and SMS autofill support.',
  ...
  composition: {
    ...
    structure: ['OTPInput.Root', 'OTPInput.Group', 'OTPInput.Slot', 'OTPInput.Separator'],
    rules: [
  ...
  design_tokens: {
    ...
    recommended: {
      colors: ['ground-300', 'ground-700', 'ground-950', 'ground-100'],
      radius: ['rounded-l', 'rounded-r'],
      typography: ['text-sm', 'font-secondary'],
    },
    allowed: {
...
export default function CustomSeparatorOTP() {
  ...
  installation: ['npx vayu-ui init', 'npx vayu-ui add otpinput'],
