export const spinnerRegistry = {
  ...
  ai_summary:
    'A WCAG 2.2 AA compliant loading indicator with three sizes (sm, md, lg), smooth CSS animation, dark mode support, and full accessibility including reduced motion preferences.',
  ...
  design_tokens: {
    used: {
      colors: ['border-brand', 'border-t-transparent'],
      border: ['rounded-full', 'border-2'],
      spacing: ['w-4', 'h-4', 'w-6', 'h-6', 'w-8', 'h-8'],
      typography: ['sr-only'],
    },
    recommended: {
      colors: ['border-brand'],
...
<div className="fixed inset-0 flex items-center justify-center bg-canvas/80">
  ...
  accessibility: {
    ...
    keyboard_support: ['No keyboard interaction required (non-interactive element)'],
    aria_attributes: [
  ...
  installation: ['npx vayu-ui init    # Add Theme CSS if not added', 'npx vayu-ui add spinner'],
