...
affixRegistry,
  ...
  // ── Button (rich metadata) ─────────────────────────────────────────────────
  {
    ...
    since: "1.0.0",
    description:
      "Triggers an action or event. Supports variants, sizes, loading state, icons, and badges via a compound component API.",
    targetPath: "src/components/ui",
    fileName: "button.tsx",
    dependencies: ["clsx"],
    registryDependencies: [],
    ...
    props: {
      ...
      loading: {
        ...
        default: "Status.IDLE",
        description:
          "Pass Status.PENDING to show spinner and disable interaction",
      },
      loadingText: {
        type: "string",
        required: false,
      ...
      type: {
        ...
        default: '"button"',
        description:
          "Native HTML button type. Always set to submit inside forms",
      },
      children: {
  ...
  <Button type="submit" loading={isSubmitting ? Status.PENDING : Status.IDLE} fullWidth>
    ...
    a11y: {
      ...
      requiredAttrs: {
        "aria-label":
          "Required when button has no visible text (icon-only buttons)",
      },
      ...
      focusManagement:
        "Focus remains on button after click. Move focus manually if button triggers navigation.",
    },
    composition: {
      parts: {
  ...
  // ── Modal (rich metadata) ──────────────────────────────────────────────────
  {
    ...
    since: "1.0.0",
    description:
      "Accessible dialog overlay rendered in a portal. Supports focus trap, Escape key, scroll lock, and multiple semantic variants via a compound component API.",
    targetPath: "src/components/ui",
    fileName: "modal.tsx",
    dependencies: ["clsx", "lucide-react"],
    registryDependencies: [],
    ...
    props: {
      ...
      initialFocus: {
        ...
        required: false,
        description:
          "Ref to focus when modal opens instead of auto-detecting first interactive",
      },
      restoreFocus: {
        type: "boolean",
        required: false,
    ...
    events: {
      onOpenChange: {
        ...
        description:
          "Fires when the modal requests a state change (Escape, overlay click, close button)",
        controlledBy: "open",
      },
    },
    states: {
      open: {
        trigger: "open={true}",
        visualChange:
          "Portal renders, backdrop fades in, panel slides up with zoom",
...
<Modal open={open} onOpenChange={setOpen}>
  ...
  <Modal.Content>...</Modal.Content>
    ...
    a11y: {
      ...
      focusManagement:
        "On open: auto-focuses first interactive element (skipping close button). On close: restores focus to the element that triggered the modal.",
    },
    composition: {
      parts: {
        Modal: "Root — manages state, portal, focus trap, scroll lock",
        "Modal.Content":
          "Panel container — sets size, variant border, and role=dialog",
        "Modal.Header":
          "Top section with optional variant icon and close button",
        ...
        "Modal.Close":
          "Custom close trigger (use asChild to wrap your own element)",
        "Modal.Trigger":
          "Optional trigger (use asChild to wrap your own button)",
      },
  ...
  // ── TextInput (rich metadata) ──────────────────────────────────────────────
  {
    ...
    since: "1.0.0",
    description:
      "Fully-featured compound text input with support for text, email, password, number, tel, url, and search types. Includes label, field, error, description, and character count sub-components.",
    targetPath: "src/components/ui",
    ...
    props: {
      ...
      required: {
        ...
        default: "false",
        description:
          "Marks field as required (adds * to label, sets aria-required)",
      },
      loading: {
        type: "boolean",
        required: false,
    ...
    states: {
      ...
      error: {
        ...
        visualChange:
          "Border turns error-500, ring-4 error tint; ErrorMessage renders",
        ariaAttr: 'aria-invalid="true"',
      },
      warning: {
        trigger: 'validationState="warning"',
        visualChange:
          "Border turns warning-500, ring-4 warning tint; WarningMessage renders",
      },
      success: {
        trigger: 'validationState="success"',
        visualChange:
          "Border turns success-500, ring-4 success tint; SuccessMessage renders",
      },
      loading: {
...
<TextInput.Root value={val} onChange={setVal}>
  ...
  <TextInput.CharacterCount maxLength={160} />
    ...
    a11y: {
      ...
      focusManagement:
        "Focus goes directly to the <input> element. Label is linked via htmlFor.",
      liveRegion:
        "ErrorMessage uses role=alert + aria-live=polite for screen reader announcements.",
    },
    composition: {
      parts: {
        "TextInput.Root":
          "Context provider — manages value, focus, IDs, and validation state",
        ...
        "TextInput.CharacterCount":
          "Character counter (configurable threshold)",
