...
export type RegistryItemType = 'component' | 'hook';
...
export type HookCategory =
  | 'state' // useLocalStorage, useList, useMap, useSet, useQueue
  | 'lifecycle' // useIsMount, useRenderCount
  | 'dom' // useOnClickOutside, useMeasure, useElementPosition, useHover
  | 'animation' // useCountdown, useDebounce, useThrottle, useTimeout, useIntervalWhen
  | 'sensor' // useBatteryStatus, useNetworkStatus, useWindowSize, useScrollPosition
  | 'side-effect' // useCopyToClipboard, useLockBodyScroll, usePageLeave, useConfirmExit
  | 'async' // useIndexedDB
...
export interface PeerComponentSuggestion {
...
export type RegistryEntry = ComponentRegistryEntry | HookRegistryEntry;
...
export interface ComponentVariantsResponse {
  slug: string;
  name: string;
  variants?: VariantDefinition;
  sizes?: SizeDefinition;
...
export interface ComponentEventsResponse {
  slug: string;
  name: string;
  events: EventDefinition[];
}
...
export interface ComponentExampleResponse {
  slug: string;
  name: string;
  examples: CodeExample[];
}
...
export interface ComponentA11yResponse {
  slug: string;
  name: string;
  a11y: A11yDefinition;
}
...
export interface ComponentDoNotResponse {
  slug: string;
  name: string;
  doNot: DoNotRule[];
}
...
export interface ComponentPeerComponentsResponse {
  slug: string;
  name: string;
  peerComponents: PeerComponentSuggestion[];
}
...
export interface ComponentCompositionResponse {
  slug: string;
  name: string;
  rootComponent: string;
  subComponents: SubComponentDefinition[];
