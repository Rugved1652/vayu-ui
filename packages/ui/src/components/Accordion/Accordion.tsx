...
import { AccordionProps, AccordionContext } from './types';
...
const Accordion: React.FC<AccordionProps> & {
  ...
  const panelRefs = useRef<Map<string, React.RefObject<HTMLDivElement | null>>>(new Map());
  ...
  const registerPanel = useCallback((id: string, ref: React.RefObject<HTMLDivElement | null>) => {
  ...
  return (
    <AccordionContext.Provider value={contextValue}>
      <div
        className={cn('border border-border rounded-surface overflow-hidden bg-surface', className)}
      >
        {children}
      </div>
