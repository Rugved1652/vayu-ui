...
function DraggableRoot({
  ...
  const [internalContainers, setInternalContainers] = useState<ContainersMap>(
    defaultContainers ?? {},
  );
  ...
  const setContainersState = useCallback(
    (next: ContainersMap) => {
      if (controlledContainers === undefined) setInternalContainers(next);
  ...
  function resetDrag() {
    ...
    srcContainerRef.current = null;
  }

  /* ---- registration ---- */
  const registerItem = useCallback((v: string, el: HTMLDivElement) => {
    itemRefsMap.current.set(v, el);
  }, []);
  const unregisterItem = useCallback((v: string) => {
    itemRefsMap.current.delete(v);
  }, []);
  const registerContainer = useCallback((id: string, el: HTMLDivElement) => {
    containerRefsMap.current.set(id, el);
  }, []);
  const unregisterContainer = useCallback((id: string) => {
    containerRefsMap.current.delete(id);
  }, []);
  ...
  const startDrag = useCallback(
    (id: string, containerId: string | null, offsetX?: number, offsetY?: number) => {
      ...
      if (isMulti && containerId) {
        ...
        srcContainerRef.current = containerId;
        setOverContainerId(containerId);
        overContainerRef.current = containerId;
        const ci = (containers[containerId] ?? []).indexOf(id);
        setOverIndex(ci);
        overIndexRef.current = ci;
      } else {
  ...
  const moveDrag = useCallback(
    (x: number, y: number) => {
      if (isMulti) {
        ...
        if (tc) {
          setOverContainerId(tc);
          overContainerRef.current = tc;
          const cItems = containers[tc] ?? [];
          const ci = getClosestIndex(
            x,
            y,
            itemRefsMap.current,
            cItems,
  ...
  const endDrag = useCallback(() => {
    ...
    if (isMulti && srcContainerRef.current) {
      ...
      if (tgtId !== srcId) {
        ...
        fromIndex !== -1 &&
        fromIndex !== overIndexRef.current &&
        overIndexRef.current >= 0
      ) {
        recordRects();
        setContainersState({
          ...containers,
          [srcId]: arrayMove(srcList, fromIndex, overIndexRef.current),
        });
      ...
      if (isMulti) {
        for (const [cId, cItems] of Object.entries(containers)) {
          if (cItems.includes(id)) {
            setSourceContainerId(cId);
            srcContainerRef.current = cId;
            setOverContainerId(cId);
            overContainerRef.current = cId;
  ...
  (ctx as any)._setLayout = (l: 'list' | 'grid', c: number) => {
    setLayout(l);
    setColumns(c);
  };
