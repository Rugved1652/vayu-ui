...
function NavbarRoot({ className, children, mainContentSelector = 'main', ...props }: NavbarProps) {
  ...
  return (
    <NavbarContext.Provider value={{ mobileOpen, setMobileOpen, closeMenu, menuId, triggerId }}>
      <nav
        ...
        className={cn('relative z-40 w-full', 'bg-surface border-b border-border', className)}
        {...props}
      >
        {children}
      </nav>
