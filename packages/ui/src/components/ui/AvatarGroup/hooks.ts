...
export function useSpacing(spacing: 'tight' | 'normal' | 'loose' | number): number {
  return useMemo(() => {
...
export function useKeyboardNavigation() {
  return useCallback((event: KeyboardEvent<HTMLDivElement>) => {
    if (event.key === 'ArrowRight' || event.key === 'ArrowLeft') {
      ...
      const focusableElements = Array.from(event.currentTarget.querySelectorAll('[role="button"]'));
