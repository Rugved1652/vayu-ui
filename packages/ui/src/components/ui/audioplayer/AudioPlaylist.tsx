...
export const AudioPlayerPlaylist = forwardRef<HTMLDivElement, HTMLAttributes<HTMLDivElement>>(
  ({ children, className, ...props }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className={clsx(
          'flex flex-col flex-1 overflow-y-auto divide-y divide-border/20',
          className,
        )}
        {...props}
      >
...
export const AudioPlayerTrack = forwardRef<HTMLDivElement, AudioPlayerTrackProps>(
  ({ src, title, artist, poster, className, 'data-index': indexObj, ...props }, ref) => {
    ...
    return (
      ...
      >
        <div className="w-10 h-10 rounded shrink-0 bg-muted flex items-center justify-center overflow-hidden">
          {poster ? (
            <img src={poster} alt={title || 'Track'} className="w-full h-full object-cover" />
          ) : (
            <Music className="w-5 h-5 opacity-50" />
          )}
        </div>
        <div className="flex flex-col flex-1 min-w-0">
          <span className={clsx('truncate text-sm font-semibold', isActive ? 'text-brand' : '')}>
            {title || 'Unknown Title'}
          </span>
          {artist && <span className="truncate text-xs text-muted-content">{artist}</span>}
        </div>
        {isActive && isPlaying && (
          <div className="w-2 h-2 rounded-full bg-brand animate-pulse mr-2" />
        )}
      </div>
    );
  },
