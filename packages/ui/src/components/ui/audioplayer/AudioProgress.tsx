...
export const AudioPlayerSeek = forwardRef<HTMLDivElement, HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => {
    ...
    return (
      ...
      >
        <div className="absolute w-full h-1.5 bg-black/10 dark:bg-white/10 rounded-full overflow-hidden transition-all group-hover:h-2">
          <div
            className="absolute top-0 bottom-0 left-0 bg-black/20 dark:bg-white/20 transition-all"
            style={{ width: `${bufferProgress}%` }}
          />
          <div
            className="absolute top-0 bottom-0 left-0 bg-brand transition-all"
