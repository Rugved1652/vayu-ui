...
export const AudioPlayerTrackInfo = forwardRef<HTMLDivElement, HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className={clsx('flex flex-row items-center gap-4 py-2 px-1', className)}
        {...props}
      >
        {currentTrack.poster && (
        ...
        <div className="flex flex-col justify-center min-w-0">
          ...
          {currentTrack.artist && (
            <div className="text-text-sm text-muted-content truncate">{currentTrack.artist}</div>
          )}
        </div>
      </div>
    );
