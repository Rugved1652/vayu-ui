...
export const AudioPlayerWaveform = forwardRef<
  HTMLCanvasElement,
...
>(({ className, ...props }, ref) => {
  ...
  useEffect(() => {
    ...
    const seedStr =
      typeof currentTrack.src === 'string' ? currentTrack.src : 'fallback';
  ...
  useEffect(() => {
    ...
    let frameId: number;
    const render = () => {
      const { currentTime: time, duration: dur } = timeRef.current;
      ...
      barsRef.current.forEach((amp, i) => {
        ...
        ctx.fillStyle =
          barPercent <= progress ? brandColor : unplayedColor;
