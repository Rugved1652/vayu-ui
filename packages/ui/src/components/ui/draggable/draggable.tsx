...
function DraggableListInner<T extends DraggableItem>(
  ...
  const { draggedId, overId, itemRefs, handleDragStart } = useDragOperations(items, onReorder);
  ...
  const displaced = useMemo(() => {
    ...
    if (fromIdx === -1 || toIdx === -1) return new Map<string, { x: number; y: number }>();
  ...
  // Context
  const ctx = useMemo<Ctx>(() => ({ draggedId, overId }), [draggedId, overId]);
  ...
  const buildDragHandleProps = useCallback(
    (itemId: string): DragHandleProps => ({
      ...
      'aria-pressed': keyboardGrabbedId === itemId ? true : undefined,
      onKeyDown: (e) => handleItemKeyDown(itemId, e),
      onMouseDown: (e) => {
        e.preventDefault();
        handleDragStart(itemId, e.clientX, e.clientY);
      },
      onTouchStart: (e) => handleDragStart(itemId, e.touches[0].clientX, e.touches[0].clientY),
    }),
  ...
  return (
    <DraggableContext.Provider value={ctx}>
      ...
      <div id={instructionId} className="sr-only">
        Press Space to grab an item. Use arrow keys to move it. Press Space again to drop, or Escape
        to cancel.
      </div>
      ...
      <div
        ...
        className={clsx(
          isGrid
            ? 'grid gap-2'
            : clsx('flex gap-2', isVertical ? 'flex-col' : 'flex-row flex-wrap'),
          className,
        )}
        style={
          isGrid
...
const DraggableList = forwardRef(DraggableListInner) as <T extends DraggableItem>(
