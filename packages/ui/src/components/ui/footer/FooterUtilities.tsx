...
function FooterCopyright({ children, className = '', ...props }: FooterCopyrightProps) {
  return (
    <div
      className={cn('text-muted-content', 'text-sm transition-colors duration-300', className)}
      {...props}
    >
      {children}
    </div>
...
function FooterDivider({ className = '', ...props }: FooterDividerProps) {
  return (
    <hr
      className={cn('border-border', 'my-8 transition-colors duration-300', className)}
      role="separator"
