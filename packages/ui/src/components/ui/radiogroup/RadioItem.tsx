...
const RadioItem = forwardRef<HTMLLabelElement, RadioItemProps>(
  ({ value, label, description, disabled: itemDisabled = false, className, ...props }, ref) => {
      ...
      >
        ...
        <div
          className={cn(
            ...
            isChecked ? 'border-brand bg-brand' : 'border-field',
          ...
          aria-hidden="true"
        >
          {isChecked && <div className={cn('w-2.5 h-2.5 rounded-full bg-brand-content')} />}
        </div>
        ...
        {(label || description) && (
          <div className="flex-1 min-w-0">
            ...
            {description && (
              <p className={cn('text-sm font-secondary', 'text-muted-content', 'mt-0.5')}>
                {description}
              </p>
            )}
          </div>
