...
export const RateStar: React.FC<RateStarProps> = ({ index }) => {
  ...
  const shouldShowFocusRing =
    isContainerFocused && !isDisabled && !readOnly && starIndex === activeStarIndex;

  const emptyClasses = 'text-muted';
  const filledClasses = error ? 'text-destructive fill-destructive' : 'text-warning fill-warning';
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 left-0">{renderIcon(icon, emptyClasses, false)}</div>
        ...
        <>
          ...
          <span
            className={`absolute top-0 ${
              allowHalf ? 'right-0 w-1/2' : 'left-0 w-full'
