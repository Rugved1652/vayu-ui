...
export const useVideoPlayer = (): VideoPlayerContextValue => {
  ...
  if (!ctx) {
    throw new Error('VideoPlayer.* components must be rendered inside <VideoPlayer.Root>.');
  }
  return ctx;
};
...
const VideoPlayerRoot = forwardRef<{ videoElement: HTMLVideoElement | null }, VideoPlayerRootProps>(
  (
    {
      children,
      className,
    ...
    const toggleSubtitles = useCallback(() => {
      setShowSubtitles((prev) => {
        ...
        if (videoRef.current && availableTracks.length > 0) {
          const track = availableTracks[activeTrackIndex >= 0 ? activeTrackIndex : 0];
    ...
    const setActiveTrack = useCallback(
      (index: number) => {
        ...
        availableTracks.forEach((track, i) => {
          track.mode = i === index && showSubtitles ? 'showing' : 'hidden';
        });
      },
    ...
    return (
      <VideoPlayerContext.Provider value={ctx}>
        <div
          ...
          onMouseLeave={() => {
            if (autoHideControls && isPlaying && !isHoveringControls) setShowControls(false);
          }}
          {...divProps}
        >
          {children}
