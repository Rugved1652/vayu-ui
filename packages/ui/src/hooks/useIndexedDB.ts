...
export const useIndexedDB = <T = unknown>({
    ...
    const put = useCallback(
        async (value: T): Promise<IDBValidKey> => {
            try {
                return await txn<IDBValidKey>(ensureDB(), storeName, "readwrite", (s) =>
                    s.put(value)
                );
            } catch (err) {
    ...
    const get = useCallback(
        async (key: IDBValidKey): Promise<T | undefined> => {
            try {
                return await txn<T | undefined>(ensureDB(), storeName, "readonly", (s) =>
                    s.get(key)
                );
            } catch (err) {
                setError(err as Error);
    ...
    const remove = useCallback(
        async (key: IDBValidKey): Promise<void> => {
            try {
                await txn<undefined>(ensureDB(), storeName, "readwrite", (s) =>
                    s.delete(key)
                );
            } catch (err) {
                setError(err as Error);
